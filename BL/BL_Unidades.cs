...

    {
                ...
                using (IDataReader reader = db.ExecuteReader(cmd))
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        for (int i = 2; i < cols; i++)
                        {
                            ...
                            Actualizar_BD(NSE_, "SUMA", "UNIDADES", Mercado_, "0. Cosmeticos", "UNIDADES (%)",
                    ...
                    for (int i = 0; i < sdata48Meses_x_Capital_Provincia.GetLength(1); i++)
                    {
                        ...
                        Actualizar_BD("Cosmeticos", "SUMA", "UNIDADES", "1. Capital", "0. Cosmeticos", "UNIDADES (%)",
        ...
        public void Leer_Ultimos_48_Meses_CIUDAD_CATEGORIA(string Cab, string xPeriodos)
        {
            ...
            string consulta = @"SELECT REGION, PARAMETROCATEGORIA, " + @Cab +
                " FROM ( SELECT PERIODO, REGION, PARAMETROCATEGORIA, FACTOR_UNIDAD/1000000 AS FACTOR_UNIDAD FROM BASE_REGIONES " +
                ...
                using (IDataReader reader = db.ExecuteReader(cmd))
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        for (int i = 2; i < cols; i++)
                        {
                            ...
                            Actualizar_BD(NSE_, "SUMA", "UNIDADES", Ciudad_, Mercado_, "UNIDADES (%)",
                                "MENSUAL", Periodo, valor, int.Parse(BD_Zoho.sCabecera48Meses[i - 2].Substring(0, 4)));
                        }
                        rows++;
                    }
                    ...
                    for (int id = 0; id < 3; id++)
                    {
                        ...
                        for (int i = 0; i < sdata48Meses_x_Region_Categoria_Mes.GetLength(1); i++)
                        {
                            ...
                            Actualizar_BD(NSE_, "SUMA", "UNIDADES", "0. Consolidado", Mercado_, "UNIDADES (%)", "MENSUAL", Periodo,
                                sdata48Meses_x_Region_Categoria_Mes[id, i] + sdata48Meses_x_Region_Categoria_Mes[id + 3, i], 
        ...
        public void Leer_Ultimos_48_Meses_CIUDAD_CANAL_VENTA(string Cab, string xPeriodos)
        {
            ...
            string consulta = @"SELECT REGION, COD_MOD_VENTA, " + @Cab +
                " FROM ( SELECT PERIODO, REGION, COD_MOD_VENTA, FACTOR_UNIDAD/1000000 AS FACTOR_UNIDAD FROM BASE_REGIONES " +
                "WHERE IDPERIODO IN (" + @xPeriodos + ") AND IDMONEDA = 2  ) AS SourceTable " +
                "PIVOT (SUM(FACTOR_UNIDAD) " +
                "FOR PERIODO IN (" + @Cab + ")) AS pvt " +
                ...
                using (IDataReader reader = db.ExecuteReader(cmd))
                {
                    ...
                    for (int id = 0; id < 2; id++)
                    {
                        ...
                        for (int i = 0; i < sdata48Meses_x_Region_Modalidad_Mes.GetLength(1); i++)
                        {
                            ...
                            Actualizar_BD(NSE_, "SUMA", "UNIDADES", "0. Consolidado", Mercado_, "UNIDADES (%)", "MENSUAL", Periodo,
                                sdata48Meses_x_Region_Modalidad_Mes[id, i] + sdata48Meses_x_Region_Modalidad_Mes[id + 2, i], 
                                int.Parse(BD_Zoho.sCabecera48Meses[i].Substring(0, 4)));
                        }
                    }
                }
